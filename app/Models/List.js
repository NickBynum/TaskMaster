...
export default class List {
  ...
  get Template() {
    return /*html*/ `
<div class="col-12 col-md-3 d-flex justify-content-around my-3 m-auto">
  <div class="row w-100 text-center bg-primary text-light py-1">
    ...
    <i type="button" class="col-2 fa fa-times text-danger text-right align-self-center"
      onclick="app.listController.delete('${this.id}')"></i>
    <div class="col-12">
      <div class="card-body bg-light text-secondary">
        <dl class="text-secondary">${this.Task}</dl>
        <form onsubmit="app.listController.addTask(event, '${this.id}')" class="input-group">
  ...
  get Task() {
    let taskTemplate = "";
        this.tasks.forEach((task, index) => taskTemplate += /*html*/`
<dd class="list-group-item">
  <input type="checkbox">
  <span id="eachTask">${task}</span>
  <i type="button" class="col-2 fa fa-times text-danger text-right align-self-center"
    onclick="app.listController.deleteTask('${index}', '${this.id}')"></i>
</dd>
`)
return taskTemplate
  }
